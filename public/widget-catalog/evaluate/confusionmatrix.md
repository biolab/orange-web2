...
The widget usually gets the evaluation results from [Test & Score](/widget-catalog/evaluate/../evaluate/testandscore); an example of the schema is shown below.
...
2. In *Show*, we select what data we would like to see in the matrix.
   ...
   ![](/widget-catalog/evaluate/images/ConfusionMatrix-propTrue.png)
...
![](/widget-catalog/evaluate/images/ConfusionMatrix-Schema.png)

[Test & Score](/widget-catalog/evaluate/../evaluate/testandscore) gets the data from [File](/widget-catalog/evaluate/../data/file) and two learning algorithms from [Naive Bayes](/widget-catalog/evaluate/../model/naivebayes) and [Tree](/widget-catalog/evaluate/../model/tree). It performs cross-validation or some other train-and-test procedures to get class predictions by both algorithms for all (or some) data instances. The test results are fed into the **Confusion Matrix**, where we can observe how many instances were misclassified and in which way.

In the output, we used [Data Table](/widget-catalog/evaluate/../data/datatable) to show the instances we selected in the confusion matrix. If we, for instance, click *Misclassified*, the table will contain all instances which were misclassified by the selected method.

The [Scatter Plot](../visualize/scatterplot.md) gets two sets of data. From the [File](/widget-catalog/evaluate/../data/file) widget it gets the complete data, while the confusion matrix sends only the selected data, misclassifications for instance. The scatter plot will show all the data, with bold symbols representing the selected data.
